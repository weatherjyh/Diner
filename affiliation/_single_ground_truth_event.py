...
def affiliation_precision_distance(Is = [(1,2),(3,4),(5,6)], J = (2,5.5)):
    ...
    if all([I is None for I in Is]): # no prediction in the current area
        d = math.nan # undefined
    else:
        d = (sum([integral_interval_distance(I, J) for I in Is]) / sum_interval_lengths(Is))
    # print(f"Is: {Is}, J: {J}, d: {d}")
    return d
...
def affiliation_recall_distance(Is = [(1,2),(3,4),(5,6)], J = (2,5.5), bound_mode=None):
    """
    ...
    if len(Is) == 0: # there is no prediction in the current area
        return(math.inf)
    E_gt_recall = get_all_E_gt_func(Is, (-math.inf, math.inf), bound_mode)  # here from the point of view of the predictions
...
def affiliation_recall_proba(Is = [(1,2),(3,4),(5,6)], J = (2,5.5), E = (0,8), bound_mode=None):
    """
    ...
    if len(Is) == 0: # there is no prediction in the current area
        return(0)
    E_gt_recall = get_all_E_gt_func(Is, E, bound_mode) # here from the point of view of the predictions
    Js = affiliation_partition([J], E_gt_recall) # partition of J depending of proximity with Is
